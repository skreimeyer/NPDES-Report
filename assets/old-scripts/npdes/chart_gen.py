...
class DataProcessor:
    ...
    def reshape_table(self,):
        ...
        dates = [i.replace('29','28') for i in dates] # Leap year is a problem, apparently
...
##        locations = self.dmr[self.dmr.columns[0]].unique()
        self.dmr = pd.melt(self.dmr,id_vars=self.dmr.columns[:4],var_name="Date",value_name="Value")
        self.dmr['Date'] = pd.to_datetime(self.dmr['Date'],format='%m/%d/%Y') # Dates are now DTG
...
##        return locations,parameters
    ...
    def simplify_names(self,):
        ...
        #Rename columns
        self.dmr.columns = ['Location','Parameter','Unit','Frequency','Date','Value']
        return None
    ...
    def get_base_limits(self):
        ...
        base_limits = {}
        for p in parameters:
            base_limits[p] = self.dmr['Value'][self.dmr['Parameter']==p].quantile(q=0.95)
        return base_limits
    ...
    def gen_limits(self,hardness):
        ...
        limits['Pb']=np.e**(1.273*np.log(hardness)-1.460)*(1.46203-np.log(hardness)*0.145712)
        limits['Hg']=2.4*0.85
        ...
        limits['Cd']=np.e**(1.28*np.log(hardness)-3.828)*(1.36672-np.log(hardness)*0.041838)
        limits['E.Coli']=410
    ...
    def normalize(self,):
        ...
        for d in dates:
            for l in locations:
                results = self.dmr[(self.dmr['Location']==l)&
                                            (self.dmr['Date']==d)]
                ...
                for p in params:
                    ...
                    newval = results.loc[results.Parameter==p,'Value']*1./upper_limit
                    self.dmr.loc[(self.dmr['Location']==l)&
                                 (self.dmr['Date']==d)&
    ...
    def format_out(self,start,end=datetime.now()):
        ...
        target_params = [
            'Flow','BOD','pH','TSS','TDS','Oil','N','P','CaCO3','Ag','Zn','Cd',
            'Pb','Cu','Streptococci','E.Coli']
        ...
        for q in quarters:
            ...
            for l in locations:
                ...
                for p in target_params:
                    values.append(float(data['Value'][(data['Date']==q)&
                                    (data['Location']==l)&
                                    (data['Parameter']==p)]))
...
if __name__ == '__main__':
    ...
    fig.text(0.5, 0.965, 'Water Quality Parameters For 2017',
